...
// Deja UN solo nodo en el programa (borra el resto con service_role). Con Claude
// real dividir-nodos suele crear varios nodos, y el pool inicial COMPLETO de un
// programa multi-nodo supera los límites de la plataforma (504 IDLE_TIMEOUT a los
// 150 s / 546 WORKER_RESOURCE_LIMIT). En producción la siembra es idempotente por
// nodo y el retry la completa; el contrato que probamos acá no depende de la
// cantidad de nodos, así que un nodo alcanza (y gasta menos API).
async function dejarUnNodo(nodos) {
  const extras = nodos.slice(1).map((n) => n.id);
  if (extras.length) {
    await fetch(`${URL}/rest/v1/nodo?id=in.(${extras.join(',')})`, { method: 'DELETE', headers: srHeaders() });
  }
  return nodos[0];
}
...
test('pool inicial: siembra >0 y ≤36 por nodo (shape validado), solo docente dueña, idempotente', { skip }, async () => {
  ...
  try {
    ...
    const nodo = await dejarUnNodo(nodos);
    ...
    assert.ok(generados <= POOL_INICIAL_NODO, `no más de ${POOL_INICIAL_NODO} para el nodo único (generados=${generados})`);
    ...
    const ej = await (await fetch(`${URL}/rest/v1/ejercicio?nodo_id=eq.${nodo.id}&select=enunciado,opciones,correcta,tipo,dificultad`, { headers: srHeaders() })).json();
    ...
    const ejDespues = await (await fetch(`${URL}/rest/v1/ejercicio?nodo_id=eq.${nodo.id}&select=id`, { headers: srHeaders() })).json();
...
test('reposición: la dueña agrega lotes de >0 y ≤12 que se acumulan en el pool', { skip }, async () => {
  ...
  try {
    ...
    const contar = async () => (await (await fetch(`${URL}/rest/v1/ejercicio?nodo_id=eq.${nodo.id}&select=id`, { headers: srHeaders() })).json()).length;
    ...
    const r1 = await callFnAuth('generador-ejercicios', { nodo_id: nodo.id }, doc.access_token);
    ...
    const r2 = await callFnAuth('generador-ejercicios', { nodo_id: nodo.id }, doc.access_token);
...
test('reposición como alumno: publicada de su escuela sí; borrador u otra escuela, no', { skip }, async () => {
  ...
  try {
    ...
    const rPool = await callFnAuth('generador-ejercicios', { programa_id }, doc.access_token); // pool inicial (pide 36 para el nodo único)
    await esperarStatus(rPool, 200, 'pool inicial');

    // (b) con la materia todavía en BORRADOR, el alumno NO puede reponer
    const rBorrador = await callFnAuth('generador-ejercicios', { nodo_id: nodo.id }, alumno.access_token);
    ...
    const poolIds = (await (await fetch(`${URL}/rest/v1/ejercicio?nodo_id=eq.${nodo.id}&select=id`, { headers: srHeaders() })).json()).map((e) => e.id);
    assert.ok(poolIds.length >= 16, `precondición: el pool inicial dejó >=16 en el nodo (dejó ${poolIds.length}; Claude vino muy corto)`);
    const rOk = await callFnAuth('generador-ejercicios', { nodo_id: nodo.id }, alumno.access_token);
    ...
    const sesRes = await fetch(`${URL}/rest/v1/sesion`, {
      ...
      body: JSON.stringify([{ alumno_id: alumno.id, nodo_id: nodo.id, aciertos: responder, total: responder }]),
    });
    ...
    const rOk2 = await callFnAuth('generador-ejercicios', { nodo_id: nodo.id }, alumno.access_token);
    ...
    const rOtra = await callFnAuth('generador-ejercicios', { nodo_id: nodo.id }, alumnoOtra.access_token);
