...
## ADR-011 — Alumno golondrina: matrícula fuente de verdad con caché en `perfil`
**Contexto:** el vínculo alumno↔colegio vivía desnormalizado en `perfil` (`docente_id/aula_id/escuela_id`); si la familia migraba, la única operación era borrar al alumno y su historia. Hacían falta transferencias con consentimiento parental (Ley 25.326), derechos ARCO, y tenancy de dos niveles (instituciones con pools de licencias). Restricción: 7 policies RLS y ~40 archivos leen esas columnas — un refactor a JOINs multiplicaría bugs de scoping con datos de menores. Spec: `2026-08-10-alumno-golondrina-design.md`.
**Decisión:** `matricula` (0022) es la fuente de verdad; las columnas de `perfil` quedan como **caché de la matrícula activa** mantenido por trigger, con triple candado: escritura solo vía RPCs `matricula_abrir`/`matricula_cerrar`, trigger `matricula_sync` único escritor del caché (GUC `edutia.matricula_sync`), y `perfil_guard` que rechaza cualquier otro escritor (tapa además el self-update de `perfil` de 0002). Estados del alumno con máquina en DB (`baja` terminal, solo vía ARCO); cerrar matrícula revoca el login del chico. **Sin consentimiento no existe transferencia**: `matricula_abrir` exige consentimiento vigente para toda reapertura y un CHECK en `transferencia` lo hace imposible hasta por SQL directo (0023). La cancelación ARCO es **el único borrado físico del sistema** (2 pasos, solo super, snapshot anónimo previo; 0024). `institucion_admin` es tabla propia **fail-closed** (las fns admin existentes le dan 403 sin tocarse; 0025) y jamás ve alumnos individuales. `licencia` evoluciona los trials reutilizando el corte suave (vencida = solo lectura) con `acceso_calcular` v2 de firma idéntica y fallback a la rama 0018 (0026).
**Consecuencias:** cerrar matrícula corta el scoping del colegio al instante en todas las superficies sin tocar una policy; el legajo viaja con el alumno (el colegio nuevo ve la historia completa); los boletines emitidos quedan para la docente que los escribió; la reversión es barata (las columnas de `perfil` nunca se dropean y el backfill es INSERT puro). "Borrar alumno" desaparece del producto: es "dar de baja" con motivo. Deuda explícita: consentimientos `pendiente_regularizar` del backfill, visibles en Mi clase.
