...
**Caché sincronizado por `matricula_sync` (0022, ampliado en 0034):** las columnas de vínculo de `perfil` (`docente_id`/`aula_id`/`escuela_id`/`grado`) **y** `alumno_cred.aula_id`. Esa última entró en 0034: `alumno_login` exige que la credencial coincida con el aula del código tipeado, así que un cambio de aula que no la actualizara dejaba al chico sin login por ninguna de las dos aulas (aparecía en el listado de la nueva y recibía `aula_invalida`). El caché lo escribe SOLO el trigger; `perfil_guard` rechaza a cualquier otro.
