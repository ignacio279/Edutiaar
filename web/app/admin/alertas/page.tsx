'use client';
// WP7 — Alertas del operador (Dashboard admin v3): lista priorizada calculada
// on-demand por admin-crm (alertas_listar; lógica pura en
// supabase/functions/admin-crm/alertas-logica.ts). "Listo ✓" persiste la clave
// determinística en admin_alerta_atendida (alerta_atender) y esa alerta no
// vuelve nunca para ese hecho puntual — patrón de /docente/luna.
...
type Estado =
  ...
  | { modo: 'lista'; alertas: AlertaAdmin[] };
...
export default function Page() {
  ...
  useEffect(() => {
    (async () => {
      const r = await llamarAdmin<{ alertas: AlertaAdmin[] }>('admin-crm', 'alertas_listar');
      if (!r.ok) {
        setEstado({ modo: 'error', mensaje: ERRS_ADMIN[r.data.error ?? ''] ?? 'No pudimos calcular las alertas. Probá de nuevo en un ratito.' });
        return;
      }
      setEstado({ modo: 'lista', alertas: r.data.alertas ?? [] });
    })();
  }, []);
  ...
  const atender = async (a: AlertaAdmin) => {
    setEstado((prev) => (prev.modo === 'lista'
      ? { modo: 'lista', alertas: prev.alertas.filter((x) => x.clave !== a.clave) }
      : prev));
