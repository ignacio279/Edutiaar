...
**Dificultad por grado + formatos de ejercicio (2026-07-10).** Dos mejoras sobre el loop de práctica, en 4 slices verticales:
- **Dificultad por banda.** El pool inicial de 36 ahora se **sesga por banda** (`SESGO_BANDA` en `generar.ts`, `[d1,d2,d3]` por tipo): chiquitos 20/12/4, medianos 12/12/12 (= histórico), grandes 4/12/20 — total invariante 36, toda celda ≥1 (cubre dominio). Además, arranque en frío con **piso adaptativo** por banda (`pisoBanda`/`nivelAdaptativo(...,piso)` en `web/lib/practica.ts`): los grandes empiezan en dificultad 2. La escala 1-3 y el motor ELO (`dominio.ts`) quedaron **intactos**.
- **Formatos nuevos de interacción.** Antes todo era opción múltiple (el enum `tipo` solo cambiaba el redactado). Ahora `ejercicio.formato` (enum, mig. **0016** + columna `datos` jsonb, default `opciones` → los ejercicios viejos siguen igual) suma **`escribir`** (tipea), **`ordenar`** (fichas en orden) y **`unir`** (pares). Gating por banda (`FORMATOS_BANDA`): chiquitos solo `opciones`; medianos +`escribir`+`ordenar`; grandes los 4. La **corrección** vive en el módulo puro nuevo `web/lib/correccion.ts` (`esCorrecta` — única puerta; `opciones` compara EXACTO, el resto normaliza: minúsculas, sin tildes, ñ preservada, salvo flag `datos.estricto` para ortografía). Front: widget por formato en `web/components/practica/WidgetRespuesta.tsx` (todo tap, sin drag&drop); `selectChat` → `responder(RespuestaDada, ts)`; `filtrarRenderizables` protege contra skew de versiones. `correcta` siempre queda legible (en `ordenar` es la oración; en `unir`, los pares serializados) → reveal/diagnóstico/TTS sin ramas.
- **Enforcement de imagen server-side.** `parseEjercicios(input, nodoId, banda)` ahora **descarta la imagen si la banda no es chiquitos** (antes el gate era solo del prompt).
- Lógica duplicada entre codebases (`bandaDeGrado`, `normalizarTexto`, `serializarPares`) con **tests espejo** anti-drift. **209 unit verdes**, tsc + `next build` OK. **Pendiente: deploy** (mig. 0016 + `generador-ejercicios` + `evaluar-sesion` vía MCP porque el CLI da 403, + Vercel) y smoke en prod. Plan: `~/.claude/plans/ahora-quiero-entender-le-streamed-wreath.md`.
