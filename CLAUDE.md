...
**SOL con Claude real (2026-07-06).** Secret `ANTHROPIC_API_KEY` cargado en Supabase + tope de gasto en console.anthropic.com (regla 4) + `mock: true` retirado del front. Las funciones conservan el patrón `if (mock || !key)` como fallback: sin key (o pasando `mock: true` a mano) vuelven al mock. Modelos: `claude-haiku-4-5` (`sol`, `sol-chat`, `evaluar-sesion`) y `claude-sonnet-4-6` (`dividir-nodos`, `generador-ejercicios`). Edge Functions desplegadas: `sol`, `dividir-nodos`, `evaluar-sesion`, `generador-ejercicios` (+ login). Migraciones hasta `0013`.
