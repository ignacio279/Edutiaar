...
**Dashboard admin v3 (en construcción, 2026-08-06).** Spec `2026-08-05-admin-dashboard-v3-design.md` + ADR-009. Panel de operación de la plataforma en **`/admin`** (login propio, SIN links desde la app). El admin no tiene fila en `perfil`: vive en `plataforma_admin` (server-only, niveles super/operativo) y opera solo vía Edge Functions `admin-*` (una por dominio, guard `_shared/admin.ts`, auditoría en toda mutación). Migración `0018`: estados/trials/límites en `escuela`, `escuela_feature` (flags SOL/LUNA/TERRA + planes), `docente_acceso`, `uso_api` (costos), `auditoria`, RPCs `mi_acceso()`/`acceso_de()` (trial vencido = solo lectura) y vistas públicas que reemplazan el listado anon de 0004. Seed: `scripts/seed-admin.mjs`.
